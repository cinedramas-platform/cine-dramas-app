...
ability to launch their *own* branded short-drama mobile app from one shared
codebase. The consumer-facing app (vertical feed + coin economy) is the *proof*;
the white-label capability ("this becomes yours, rebranded") is the *pitch*.
...
### Producer
A creator/studio that owns short-drama content. The *buyer* in the Silhouette
model, a *content supplier* in the Hub model. Replaces "producer house" and loose
use of "client"/"customer" in prose.

### Operator
Whoever runs a Tenant instance. In **Silhouette** the Operator *is* the Producer
(1:1, their own app). In **Hub** the Operator is CineDramas/a distributor running
one shared instance for many Producers.

### Silhouette  *(MVP target)*
...
throwaway *sample* brand id, not a real Operator.
