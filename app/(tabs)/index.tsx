...
export default function HomeScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.bg, paddingTop: insets.top }}>
      ...
      >
        <WebContent>
          {/* Top Bar — desktop has the TopNav instead */}
          {!desktop && (
            <View
              style={{
                flexDirection: 'row',
          ...
          {hero && (
            <Pressable
              ...
              style={{ paddingHorizontal: 20, paddingBottom: 22, paddingTop: desktop ? 24 : 0 }}
            >
              <CineStill
                imageUrl={hero.poster_url ?? undefined}
                playbackId={hero.thumbnail_playback_id ?? undefined}
                width={SCREEN_W - 40}
                height={desktop ? 560 : 460}
            ...
            <View style={{ paddingBottom: 24 }}>
              ...
              >
                {continueWatching.map((item: WatchProgress) => {
                  ...
                  const prog = item.completed
                    ? 1
                    : Math.min(item.position_seconds / duration, 0.95);
                  return (
                    <Pressable
                      key={item.episode_id}
                      onPress={() => goToPlayer(item.episode_id)}
                      style={{ gap: 6 }}
                    >
                      <Poster
                      ...
                      <View style={{ width: 132 }}>
                        <Text
                          style={{ fontFamily: Fonts.sans600, fontSize: 11, color: Colors.ink }}
                          numberOfLines={1}
                        >
